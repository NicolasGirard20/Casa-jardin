...
const ProfesionalForm: React.FC<ProfesionalProps> = (ProfesionalProps) => {
  const [imagePreview, setImagePreview] = useState<string>(
    ProfesionalProps.downloadUrl || "../../../../public/Images/default-no-image.png",
  )
  //esta varible no es null si hubieron cambios en la imagen
  const [imagenArchivo, setImagenArchivo] = useState<File | null>(null)
  //por si el profesional borra su imagen totalmente
  const [borrarImagen, setBorrarImagen] = useState<boolean>(false)
  ...
  const onSubmit = async (data: ProfesionalSchema) => {
    ...
    let url = null
    if (borrarImagen) {
      console.log("borrando la imagen: ", data.imagen)
      //si el profesional borra la imagen, la borro del repo
      handleDeleteProfesionalImage(data.imagen)
      url = null // Asegurar que url sea null cuando se borra la imagen
    } else {
      url = imagenArchivo ? await handleImageUpload(imagenArchivo, data) : data.imagen
      console.log("imagen modificada", url)
    }
    //actualizo el profesional
    const pro = await updateProfesional(Number(data.id), {
      nombre: data.nombre,
    ...
    if (typeof pro === "string") {
      console.error("Error al actualizar el profesional:", pro)
  ...
  const handleImageChange = (file: File | null) => {
    ...
    if (file) {
      ...
      console.log("al guardar el profesional, se borra la imagen")
      setBorrarImagen(true)
      setImagePreview(NoImage.src)
  ...
  const handleImageUpload = async (file: File, data: ProfesionalSchema) => {
    ...
    await handleUploadProfesionalImage(file, filenameResult)
            ...
            <CardContent className="space-y-6 pt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  {errors.especialidad && (
                    <p className="text-destructive text-sm mt-1">{errors.especialidad.message}</p>
                  )}
                </div>
                <div>
